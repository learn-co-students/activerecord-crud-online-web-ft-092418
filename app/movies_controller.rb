...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  Movie.all.find{ |x| x.title == "Title" && x.release_date == 2000 && x.director == "Me"}
end

def can_find_using_where_clause_and_be_sorted
  # For this test return all movies released after 2002 and ordered by
  # release date descending
  Movie.all.map{|m| m if m.release_date > 2002}.compact.sort_by{|m| -m.release_date}
end
...
def can_destroy_a_single_item
  ...
  Movie.all.each do |m|
    m.title == "That One Where the Guy Kicks Another Guy Once" ? m.destroy : m.save
  end
end
